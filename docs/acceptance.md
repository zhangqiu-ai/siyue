...
2026-09-07 国际化约束：中文和英文均为必需界面语言，覆盖已支持平台。验收包含双语文案、错误/运行状态、无障碍名称、格式化和布局，详见 [UI 语言矩阵](design/ui-acceptance.md)。本轮仅增要求，未执行双语应用验收，不改变历史场景状态。

2026-09-07 新增平台验收约束：iPhone 与 iPad 必须同时适配，iPad 为未来主力，手机体验持续保障。页面/布局的尺寸、方向、窗口及键盘检查以 [UI 设备矩阵](design/ui-acceptance.md) 为准，两类设备分别记录证据；此前 iPhone 结果不覆盖 iPad，本轮未运行 iPad 验收。历史场景状态不因新增要求自动提升。
...
| AT-024 | 闭环与跨端证据 | iPhone、iPad 与 Android 核心流程分别有证据；iPhone/iPad 同时适配，尺寸与交互按 UI 设备矩阵验收；中文与英文界面分别有适用验证；桌面范围明确；未测不算通过 |
...
## AT-027 · 邮箱登录、找回密码与安全恢复

工作包：SY-005；状态：partial。设置中登录、找回密码、关闭重启恢复、退出后重启；服务器确认后显示真实状态，重置不自动登录，秘密仅进入受保护存储，本地内容不自动上传或归属。

会话与故障覆盖见[基础证据](evidence/backend-auth-client-2026-09-22.md)，桌面实际表单及 iPhone/iPad/Android 隔离原生恢复见[界面证据](evidence/backend-auth-account-ui-2026-09-22.md)。真实邮件、注册、账号空间映射、完整 UI 与真机矩阵未完成；失败史及设备差异如实保留，不提升整体账号验收状态。

空间映射的[真实服务、SQLite 与平台验证](evidence/backend-auth-account-space-2026-09-22.md)覆盖双账号、重启、旧客户端与迟到结果，并补充 Android、iPhone 和 iPad 模拟器隔离 QA 包中的本机白板账号空间验证。正式五端、真机、共享房间、图片/相机与生产服务矩阵未完成。AT-019 与 AT-027 仍为 partial，不将隔离 QA 包代替正式应用完整验收。
