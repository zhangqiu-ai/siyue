...
2026-09-07 起用于新开发；维护者已批准采用本流程。流程建立的历史范围不构成新功能授权。
...
开发前在 proposal 写唯一 `Target release: x.y.z`，并登记到 [版本计划](../planning/releases/README.md)。当前开发版本以根 package.json 为准，发布状态见对应版本计划。验收/延期时同步版本条目；归档后将计划中的提案链接改为真实归档路径，历史版本归属保留。`corepack pnpm release:check` 检查版本与活动变更登记，不能代替业务验收。

## 按需执行与记录

沿用上述标准/简化分类，不为规则去重扩大豁免。已有授权范围内持续完成实现、适用验证、必要修复与交付；只阻塞依赖未决取舍的操作。详细格式由 config.yaml 提供，根 AGENTS 只负责路由。

在可验收节点、范围变化或交付时同步状态；一次证据可被任务、工作包和版本计划引用，不复制运行日志。适用验证通过后，仅新增修改、失败或未解决风险触发重跑或扩大验证。小改按简化范围记录，重大变更仍逐项关联场景与证据，未完成关键验收不归档。
