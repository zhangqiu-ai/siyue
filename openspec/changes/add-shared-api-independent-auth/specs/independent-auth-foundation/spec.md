...
邮箱注册 MUST 先验证邮箱控制权，再原子创建主体、邮箱、密码及会话；登录 SHALL 使用邮箱与密码，未知邮箱和错误密码返回相同拒绝。新设置的密码（注册、重置、修改、首次绑定邮箱）MUST 为 6–20 个 Unicode 码点，验证已有密码（登录、重新验证、注销）时接受 6–128 个码点，使旧规则下创建的账号仍可登录；不设大小写或字符类别要求，不 trim 或截断，保留常见密码拒绝（2026-09-25 维护者决定），使用有界 Argon2id。
