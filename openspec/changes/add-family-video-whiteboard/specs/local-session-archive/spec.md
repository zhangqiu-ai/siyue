...
系统 SHALL 默认关闭录像，仅由获相应授权的家长主动开启本地录制，并在有效录制期间持续向所有参与者显示录制者及状态；录像成品有白板时 SHALL 以白板为主画面、参与者视频为小画面并包含本地及远端声音，无白板时记录多人视频及声音。同意与中途加入规则仍依 O02 冻结，未确认不得开始真实采集。

#### Scenario: Start or join a recorded session
- **WHEN** 家长请求录制或新参与者加入正在录制的房间
- **THEN** 按已批准的告知/同意流程处理，录制状态持续可见；不能以一次弹窗替代持续提示。

#### Scenario: Recording composition follows the board
- **WHEN** 在获准的录制过程中打开或关闭共享白板
- **THEN** 有白板时保留白板主画面和参与者视频小画面，无白板时记录多人视频；两种模式均包含本地及远端声音，不静默降级为仅本机摄像头或单方声音。
...
### Requirement: VWA-05 Excalidraw 完整存档
本机编辑存档 SHALL 包含版本、白板/页面标识、场景元素、图片文件及引用、白名单视图状态；浏览器存储不得成为唯一权威原件。高频保存合并且设最大等待时间；仅在本机写入成功后显示已保存。读取失败、未来版本、图片缺失或写入失败 SHALL 保留最近完整原件。

#### Scenario: 冷启动与异步保存冲突
- **WHEN** 原件尚未读取、保存请求迟到或会话已经切换
- **THEN** 初始空场景不能覆盖原件；版本或会话不匹配请求被拒绝，当前未保存内容保留并显示失败

#### Scenario: 忽略旧开发白板
- **WHEN** 本机存在旧自研白板的开发遗留记录
- **THEN** 不读取或转换该记录，不显示转换入口；仍正常读取、编辑和保存现有 Excalidraw 作品

#### Scenario: 后台和强杀边界
- **WHEN** 应用进入后台或正常退出
- **THEN** 请求处理待保存内容；系统强杀不能保证异步保存完成，只以最后确认落盘修订为恢复依据
