...
// Password length is counted in Unicode code points and never trimmed or truncated. There is no
// character-class rule. Two bounds exist on purpose: a password being chosen now (registration,
// reset, change, first email link) must be 6–20 code points, while a password presented to prove an
// existing credential (sign-in, reauthentication, deletion) is accepted at 6–128 so accounts created
// under the earlier 15–128 rule keep signing in. Maintainer decision 2026-09-25.
const pairedSurrogates = (value: string) => !/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/u.test(value);
export const NEW_PASSWORD_MIN = 6;
export const NEW_PASSWORD_MAX = 20;
const passwordWithin = (min: number, max: number) => z.string().max(256).refine(value => {
  const size = [...value].length;
  return size >= min && size <= max && pairedSurrogates(value);
}, 'password_length_or_unicode');
/** A password that proves an existing credential. */
export const accountPasswordSchema = passwordWithin(NEW_PASSWORD_MIN, 128);
/** A password being set now. */
export const newAccountPasswordSchema = passwordWithin(NEW_PASSWORD_MIN, NEW_PASSWORD_MAX);
...
export const emailRegisterConfirmSchema = z.object({...proof,...device,password:newAccountPasswordSchema,
...
export const emailPasswordResetConfirmSchema = z.object({...proof,newPassword:newAccountPasswordSchema}).strict();
...
export const emailLinkConfirmSchema = z.object({...proof,newPassword:newAccountPasswordSchema}).strict();
...
export const passwordChangeSchema = z.object({newPassword:newAccountPasswordSchema,reauthGrant:refreshTokenSchema}).strict();
