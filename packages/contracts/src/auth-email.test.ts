...
import { accountPasswordSchema, emailChallengeRequestSchema, emailRegisterConfirmSchema, emailLinkRequestSchema, emailLinkConfirmSchema, normalizeLoginEmail, registrationPolicySchema } from './auth-email.js';
test('password boundaries count Unicode code points and preserve exact spaces',()=>{
 for(const input of ['😀'.repeat(15),'学'.repeat(128),'  multi word phrase  ']) assert.equal(accountPasswordSchema.parse(input),input);
 for(const input of ['😀'.repeat(14),'学'.repeat(129),'a'.repeat(15)+'\ud800']) assert.equal(accountPasswordSchema.safeParse(input).success,false);
});
test('email normalization does not merge aliases; identity claims and malformed OTP rejected',()=>{
 ...
 const payload={challengeId:'17898741-d6e6-4e64-b76b-927c2169a3e0',requestSecret:'A'.repeat(43),code:'000012',password:'a long synthetic phrase',installationId:'test-device',platform:'ios',termsVersion:'v1',privacyVersion:'v1'};
...
test('link request binds one reauth grant and link confirm stays strict and password-only',()=>{
 ...
 for(const changed of [{reauthGrant:undefined},{reauthGrant:'A'.repeat(43)},{reauthGrant:`${grant.slice(0,36)}.${'A'.repeat(42)}`},
   {subjectId:'17898741-d6e6-4e64-b76b-927c2169a3e0'},{action:'link-identity'},{password:'a long synthetic phrase'},
 ...
 const confirm={challengeId:'17898741-d6e6-4e64-b76b-927c2169a3e0',requestSecret:'A'.repeat(43),code:'000012',newPassword:'a long synthetic phrase'};
