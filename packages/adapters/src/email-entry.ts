import { emailAddressSchema,accountPasswordSchema,type AuthClientErrorCode,type EmailChallengeResponse,type EmailChallengeRequest,type EmailPasswordResetConfirm } from '@siyue/contracts';
  ...
  async function execute(locale:'zh-CN'|'en-US',resend=false) {
    ...
    if(!pending) {
      ...
      if(state.mode==='login'||state.mode==='reset-confirm'&&!resend) {
        if(!accountPasswordSchema.safeParse(state.password).success){update({error:'password_policy'});return;}
      }
