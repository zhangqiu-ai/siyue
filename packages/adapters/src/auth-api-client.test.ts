...
test('idempotency, strict password contract, redirect rejection and expected status are enforced',async()=>{
  ...
  assert.throws(()=>api.login({email:'a@example.test',password:'short',platform:'ios',installationId:'x'}),code('invalid_request'));
...
test('password change uses action-bound reauth and a protected idempotent request',async()=>{
 ...
 await api.changePassword(token,{newPassword:'a different pw 2026',reauthGrant:grant},key);
 ...
 assert.deepEqual(JSON.parse(calls[1]!.init.body as string),{newPassword:'a different pw 2026',reauthGrant:grant});
 await assert.rejects(api.changePassword(token,{newPassword:'a different pw 2026',reauthGrant:grant},'bad-key'),code('invalid_request'));
});
...
test('email link request and confirm use the fixed authenticated routes, bearer, idempotency key and expected 202/204',async()=>{
 ...
 const confirm={challengeId:challenge.challengeId,requestSecret:challenge.requestSecret,code:'123456',newPassword:'a valid new pw 2026'};
 ...
 assert.throws(()=>api.confirmEmailLink(token,{...confirm,newPassword:'short'},key),code('invalid_request'));
 assert.equal(calls.length,2);
...
test('AUTH_EMAIL_ALREADY_LINKED keeps its own stable code and never surfaces the server message',async()=>{
 for(const operation of ['request','confirm'] as const){
  ...
  const promise=operation==='request'?api.requestEmailLink('synthetic-access',{email:'parent@example.test',locale:'en-US',reauthGrant:grant},randomUUID()):api.confirmEmailLink('synthetic-access',{challengeId:randomUUID(),requestSecret:'c'.repeat(43),code:'123456',newPassword:'a valid new pw 2026'},randomUUID());
...
test('a rolled or closed sign-up keeps its own client code instead of reading as an outage',async()=>{
 const confirmInput={challengeId:randomUUID(),requestSecret:'A'.repeat(43),code:'000012',password:'Syn-新密码-🌙7',
