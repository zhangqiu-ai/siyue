import {emailAddressSchema,newAccountPasswordSchema,registrationPolicySchema,
...
export function createEmailRegistration(actions:EmailRegistrationActions,newId:()=>string,now=Date.now) {
  ...
  async function confirmIntent():Promise<Pending|null> {
    ...
    if(!newAccountPasswordSchema.safeParse(state.password).success){update({error:'password_policy'});return null;}
