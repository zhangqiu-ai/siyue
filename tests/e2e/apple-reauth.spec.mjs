...
test('Apple-only reauth grants revoke another device or every session, and nothing wider',async({request})=>{
 ...
 for(const action of ['unlink-identity','change-password'])
  expect(await failure(await start(request,{...reauthStart,action},who.bearer))).toEqual({status:400,code:'AUTH_INVALID_REQUEST'});
 // Account deletion is password-free for Apple-only subjects and receives its own bound grant.
 const deletion=await grantFor('delete-account');
 expect((await db.app.query('SELECT subject_id,session_id,action,consumed_at FROM siyue.reauth_grants WHERE id=$1',[deletion.split('.')[0]])).rows[0])
  .toEqual({subject_id:who.subjectId,session_id:who.tokens.session.sessionId,action:'delete-account',consumed_at:null});
