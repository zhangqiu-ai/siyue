...
test('HTTP registration → mail delivery adapter → login → refresh → password reset rejects previous devices',async({request})=>{
 ...
 fx.advance(60_001);const resetProof=await challenge(request,'password/reset',email);const newPassword='reset-confirm-pw7';
...
test('HTTP change password requires matching reauth and revokes the current device',async({request})=>{
 ...
 const changeHeaders={...headers(),authorization},changedPassword='change-pw-new7',changeBody={newPassword:changedPassword,reauthGrant:grant.reauthGrant};
 ...
 const conflicting=await request.post(address+'/v1/me/password/change',{headers:changeHeaders,data:{...changeBody,newPassword:'change-pw-other8'}});
 ...
 expect((await request.post(address+'/v1/auth/email/login',{data:{email,password:changedPassword,installationId:'test',platform:'desktop'}})).status()).toBe(200);
});
