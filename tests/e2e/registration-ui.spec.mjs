...
const address='register-ui@example.test',finalAddress='register-ui-final@example.test',accountPassword='  注册-🌙-siyue  ',shortPassword='sh0rt';
...
test('real Electron registration: consent gate, released policy, wrong code, a lost answer repeated with the same key, and a real session',async({},info)=>{
 ...
 try {
  ...
  await expect(page.getByRole('alert')).toContainText('密码需为6–20个字符。');
  await page.getByLabel(zh.password,{exact:true}).fill(accountPassword);await page.getByLabel(zh.repeatPassword,{exact:true}).fill('another-register-pw');
  await page.getByRole('button',{name:zh.createAccount,exact:true}).click();
