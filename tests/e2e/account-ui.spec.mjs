...
for(const locale of ['zh','en'])test(`${locale} actual account UI: validation, reset, sign in, restart and sign out`,async({},info)=>{
 ...
 try{
  ...
  const otp=fx.cipher.open(job.payload_ciphertext,`mail:${job.id}`).code,newPassword='a new synthetic password for account UI';
  ...
  const changedPassword='another new synthetic password for account UI';
