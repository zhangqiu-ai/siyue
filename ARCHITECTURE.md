...
版本：0.1 | 日期：2026-09-05 | 状态：实施基线；本地 Mock 与桌面运行已验证，移动原生及生产同步仍待验证
...
安装阶段生成锁文件保证复现，并分别记录 Node、pnpm、Expo、React Native、React、Electron、数据库驱动与 AI SDK 的兼容证据。锁文件存在不代表三端构建或真机均通过。不得把互不相关的最新大版本强行统一。

API、事件和命令带 schemaVersion；本地升级可恢复；服务端支持明确的旧客户端兼容窗口。兼容范围由测试结果填入，不在立项时捏造。

## 9. M1 单设备验证实现

共享包编译为 ESM，移动、桌面与服务端消费相同业务契约。`packages/adapters` 的平台中立入口使用事务端口；Node驱动只在 `/node` 子路径暴露，移动用 Expo SQLite 独占事务适配。受限单设备阶段按空间存储经 schema 校验的 JSON 快照，使正式对象、活动与回执原子提交；没有接入 ORM 或生产同步。该选择可替换，原生存储 PoC 完成前 ADR-004 仍为 Validate。

目标草稿仍使用已选择的本地 Mock。2026-09-06 增加独立移动普通聊天 BYOK：本机通过 expo/fetch 调用用户配置的 HTTPS OpenAI 兼容服务，SecureStore 保存个人密钥，只发送当前会话文字；不调用正式业务写入或上传其他记录。平台密钥仍不得下发客户端，真实供应商尚未验收；没有引入 Hermes Agent 框架。详见 [工程验证](docs/BOOTSTRAP.md)。
