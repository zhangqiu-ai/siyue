...
需已有 Expo 原生工程及 Pods。**每次 `expo prebuild` 或 `pod install` 后，先运行准备脚本，再构建 QA target**：Pods 只声明正式 `Siyue` target，重新生成工程可能移除 QA target 继承的 `Pods-Siyue.release.xcconfig`。此时 QA 的 `PODS_ROOT`、Expo 模块搜索路径及 `-l"Expo"` 链接参数都会消失，`AppDelegate.swift` 报 `no such module Expo`。准备脚本检查已安装的 Pods 配置，并将其重新关联到独立 QA target；普通应用 target 不改变。模拟器权限文件仅用于 Xcode 模拟签名，不用于真机或发行。
...
账号重构路由及断点回归可在专用 QA iPad 上单独执行上面的 `test-without-building`，追加 `-only-testing:AccountAuthUITests/AccountAuthUITests/testRedesignedAccountRoutes`。用例在竖屏检查单列、横屏检查 380pt 左栏及至少 560pt 的右侧空间，再访问登录方式、设备、账号空间及退出登录。运行前确认 `xcodebuild -project apps/mobile/ios/Siyue.xcodeproj -target SiyueAccountQA -configuration Release -sdk iphonesimulator -showBuildSettings` 含 `PODS_ROOT` 和 `-l"Expo"`；缺失时在 Pods 安装完成后重跑准备脚本。

2026-09-25：在 `Siyue Deletion QA iPad`（iPad Pro 11-inch M5，iOS 26.5，arm64）运行该单用例，`build-for-testing` 和 `test-without-building` 均成功；XCTest 1/1 通过、0 失败、0 跳过。结果包为 `artifacts/account-ui-native/redesigned-account-ipad-final-1790327273.xcresult`（本机产物，可能不随 Git 分发）。用例覆盖合成账号和模拟器的账号页面与断点；不代表真机或生产服务验收。
...
新增 `testDeletionChinese` / `testDeletionEnglish`，只用于独立 QA 包和合成账号。启动 `node tests/e2e/native-deletion-server.mjs`（与原账号 fixture 使用相同 18787 端口，不能同时运行）；它建立临时主库和独立注销账本，注册正式提交路由并运行受保护清理。四个账号按 phone/pad 和 zh/en 区分，密码为 `siyue-qa-native7`。重新执行已删除账号的用例前，重启本 fixture 以创建新的合成账号；不要清理正式应用数据。
