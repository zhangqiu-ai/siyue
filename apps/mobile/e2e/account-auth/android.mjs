 ...
 if(vaultLossMode){
  ...
  for(const locale of ['zh','en']){
   ...
   if(locale==='zh'){await tap(back);await find('QA English Plan Draft');}
    ...
    await tap('Email');await command('shell','input','text','native-en@example.test');await tap('Password');await command('shell','input','text','siyue-native-test-password');
  ...
  await tap(password);await command('shell','input','text','siyue-native-test-password');await capture(`${locale}-input`);
