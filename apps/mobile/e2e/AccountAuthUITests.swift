...
final class AccountAuthUITests: XCTestCase {
    ...
    func flow(_ english: Bool) {
        ...
        Thread.sleep(forTimeInterval: 0.5)
        password.typeText("siyue-native-test-password")
        ...
        let createSpace = app.buttons[english ? "Create an account space on this device" : "在本机创建账号空间"]
        XCTAssertTrue(createSpace.waitForExistence(timeout: 20)); show(createSpace); createSpace.tap()
        let accountSpace = app.staticTexts[english ? "Current space: account space (on this device)" : "当前空间：账号空间（仅本机）"]
        ...
        let manageDevices = app.buttons[english ? "Manage signed-in devices" : "管理登录设备"]
        XCTAssertTrue(manageDevices.waitForExistence(timeout: 20)); show(manageDevices); manageDevices.tap()
        let currentDevice = app.staticTexts.containing(NSPredicate(format: "label CONTAINS %@", english ? "This device" : "本设备")).firstMatch
        ...
        let revokeCurrent = app.buttons.matching(NSPredicate(format: "label == %@ AND enabled == YES", english ? "Sign out this device" : "在此设备退出")).firstMatch
        show(revokeCurrent); revokeCurrent.tap()
        let confirmation = app.alerts.firstMatch
        XCTAssertTrue(confirmation.waitForExistence(timeout: 10), app.debugDescription)
        confirmation.buttons[english ? "Sign out this device" : "在此设备退出"].tap()
        ...
        XCTAssertTrue(email.waitForExistence(timeout: 30), "Revoked credentials must not restore a session")
    }
    func deletionFlow(_ english: Bool) {
        ...
        let password = app.secureTextFields[english ? "Password" : "密码"]
        password.tap(); password.typeText("siyue-native-test-password")
        ...
        XCTAssertTrue(verify.waitForExistence(timeout: 15), app.debugDescription); show(verify); verify.tap(); verify.typeText("siyue-native-test-password")
    ...
    func testOtherAndAllDeviceRevocation() {
        ...
        let manage = app.buttons["Manage signed-in devices"]
        ...
        let reauth = app.secureTextFields["Current email password"]
        show(reauth); reauth.tap(); reauth.typeText("siyue-native-test-password")
        let revokeOther = app.buttons["Sign out other device"]
        show(revokeOther); revokeOther.tap()
        let confirmation = app.alerts.firstMatch
        XCTAssertTrue(confirmation.waitForExistence(timeout: 10))
        confirmation.buttons["Sign out other device"].tap()
        ...
        show(reauth); reauth.tap(); reauth.typeText("siyue-native-test-password")
        let revokeAll = app.buttons["Sign out all devices"]
        show(revokeAll); revokeAll.tap()
        XCTAssertTrue(confirmation.waitForExistence(timeout: 10))
        confirmation.buttons["Sign out all devices"].tap()
        XCTAssertTrue(email.waitForExistence(timeout: 20), app.debugDescription)
        app.terminate(); app.launch()
        XCTAssertTrue(qa.waitForExistence(timeout: 30)); qa.tap()
        XCTAssertTrue(email.waitForExistence(timeout: 30), "All revoked sessions must not restore after restart")
    }
    ...
    func testMissingSecureStoreItemAfterInitializationPreservesVaultState() {
        ...
        let preserved = app.staticTexts["登录凭据无法读取，原件已保留。请联系支持，不要清空本机内容。"]
...
final class AccountWhiteboardIsolationUITests: XCTestCase {
    ...
    private let password = "siyue-native-test-password"
