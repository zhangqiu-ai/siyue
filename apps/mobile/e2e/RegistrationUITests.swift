...
final class RegistrationUITests: XCTestCase {
    ...
    private let password = "siyue-qa-register7"
    ...
    private func openAccount(_ english: Bool) {
        ...
        waitUntil(90, "The account screen must settle as signed in or signed out") {
            self.signOutButton(english).exists || self.element("account-email-entry").exists
        }
    }
    private func registerOnScreen(_ english: Bool, _ address: String) throws {
        tap(english ? "New here? Create an account" : "还没有账号？创建账号")
        let consent = element(english ? "I have read and agree to" : "我已阅读并同意")
        ...
        let code = app.textFields[english ? "Verification code" : "验证码"]
        ...
        type(english ? "Verification code" : "验证码", try issuedCode(address), into: code, secret: false)
        type(english ? "Set password" : "设置密码", password, into: app.secureTextFields[english ? "Set password" : "设置密码"], secret: true)
        type(english ? "Repeat password" : "再次输入密码", password, into: app.secureTextFields[english ? "Repeat password" : "再次输入密码"], secret: true)
    ...
    private func registrationFlow(_ english: Bool) throws {
        ...
        openAccount(english)
        XCTAssertTrue(element("account-email-entry").waitForExistence(timeout: 90), "A cleared vault must start on the sign-in entry\n\(app.debugDescription)")
    ...
    func testRegistrationCodeAutoAdvancesToPassword() throws {
        ...
        let consent = element("I have read and agree to")
        ...
        revealSoftwareKeyboard()
        let issued = try issuedCode(address)
        type("Verification code", issued, into: code, secret: false)
        XCTAssertTrue(app.secureTextFields["Set password"].waitForExistence(timeout: 20),
                      "A complete six-digit code should open the password step\n\(app.debugDescription)")
    }
}
