...
import {emptyBoard,KEY} from '@siyue/whiteboard';
...
test('mobile host switches isolated business and board namespaces only after a successful editor flush',async()=>{
 ...
 try{
  ...
  const board=host.board('a');assert.equal((await request(board,'a','load')).ok,true);
  const blank=emptyBoard();assert.equal((await request(board,'a','save',{baseRevision:0,pages:blank.pages,files:{}})).ok,true);
  assert.equal(fixture.kv.has(KEY),false);assert.ok(fixture.kv.has(`siyue.account.${a.namespace}.${KEY}`));
  ...
  assert.deepEqual((await host.client().snapshot()).goals,[]);const b=host.board('b');assert.equal((await request(b,'b','load')).value.board,null);b.dispose();
