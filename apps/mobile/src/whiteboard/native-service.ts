...
import { createService, MAX_IMAGE, KEY, type Storage as BoardStorage } from '@siyue/whiteboard';
...
export function nativeBoardService(session:string,options?:{namespace?:string;lifetimeSignal?:AbortSignal}) {
  ...
  const storage:BoardStorage={getItemSync:key=>{if(key!==KEY)throw Error('invalid_key');return Storage.getItemSync(prefix+key);},setItemSync:(key,value)=>{if(key!==KEY)throw Error('invalid_key');Storage.setItemSync(prefix+key,value);}};
  ...
  const service=createService(storage,session,source=>pick(source,options?.namespace?`whiteboard-originals-${options.namespace}`:'whiteboard-originals',owned,external));
  let disposed=false;
  return {
