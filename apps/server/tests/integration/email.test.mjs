...
test('known/unknown login failures share error; exact Unicode/spaces preserved and common passwords rejected',async()=>{
  await assert.rejects(fx.passwords.hash('correct horse battery staple'),codeIs('AUTH_PASSWORD_TOO_COMMON'));
...
test('reset commits password/version and revokes every token/grant; replay does not repeat or auto-login',async()=>{
  ...
  const data={challengeId:proof.challengeId,requestSecret:proof.requestSecret,code:proof.code,newPassword:'New 私有 🌙 passphrase 42'};const key=randomUUID();
