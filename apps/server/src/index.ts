...
async function main() {
  ...
  try {
    ...
    const app=createRuntimeApp(pool,config.identity,{registrationPolicy,trustedProxyCidrs:config.trustedProxyCidrs,sessions,
      // Keep unreleased invitation, guardianship and child-device route groups closed in 0.0.2.
      // Their stores remain available for deletion dependency checks and bounded maintenance only.
      ...(deletionReady?{deletionReady}:{}),...(deletionSubmission?{deletionSubmission}:{}),
