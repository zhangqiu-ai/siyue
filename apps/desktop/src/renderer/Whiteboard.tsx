...
import Editor, { boardStrings } from '@siyue/whiteboard/editor';
...
export function Whiteboard({onExit}:{onExit:()=>void}) {
  ...
  return <div style={{position:'fixed',inset:0,zIndex:10}}><Editor session={session} locale={locale==='en'?'en-US':'zh-CN'} strings={boardStrings(locale==='en'?'en-US':'zh-CN')} theme={theme}
