...
export function Settings() {
  ...
  return <>
    <button ref={trigger} className="icon-button settings-trigger" aria-label={t('settings')} title={t('settings')} onClick={() => dialog.current?.showModal()}>
    ...
    <dialog ref={dialog} className="settings-dialog" aria-labelledby="settings-title" onClose={() => trigger.current?.focus()}>
      <div className="section-heading"><h2 id="settings-title">{t('settings')}</h2><button className="icon-button" aria-label={t('close')} onClick={() => dialog.current?.close()}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div>
